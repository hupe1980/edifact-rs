...
fn bench_parse_reader_1mb(b: Bencher) {
    let data = one_mb();
    b.bench(|| {
        edifact_rs::from_reader(std::io::Cursor::new(black_box(data)))
            .collect::<Result<Vec<_>, _>>()
            .expect("bench fixture must be valid EDIFACT")
    });
}
...
fn realistic_pack() -> ProfileRulePack {
    ...
    {
        let tag = *tag;
        pack = pack.with_rule_fn(move |segments, issues| {
...
fn bench_validate_profile_pack_small(b: Bencher) {
    ...
    let ctx = ValidationContext::builder()
        ...
        .build();
    b.bench(|| ctx.validate(black_box(&segs)).total_issues());
}
...
///
/// Uses the conformant single-interchange fixture and enables envelope
/// validation, because an ingest pipeline checks the envelope — and on the
/// repeated-interchange fixture `validate_envelope` aborts at the second `UNB`,
/// so "end to end" would have stopped a few segments in.
#[divan::bench]
fn bench_parse_and_validate_1mb(b: Bencher) {
    let data = one_mb_interchange();
    let ctx = ValidationContext::builder()
        .with_envelope_validation()
        .with_message_type("ORDERS")
        .with_profile_pack(realistic_pack())
    ...
    b.bench(|| {
        let segs: Vec<_> = from_bytes(black_box(data))
            .collect::<Result<Vec<_>, _>>()
            .expect("bench fixture must be valid EDIFACT");
        ctx.validate(&segs).total_issues()
    });
}
