...
//! # Key APIs
//!
//! - [`message_windows`] — byte-slice source, produces owned windows
...
use edifact_rs::{
    EdifactDeserialize, EdifactSerialize, message_windows, message_windows_from_reader,
};

// ── 1. Manual approach: inspect windows as raw segment slices ─────────────────

fn count_messages_in_interchange(input: &[u8]) -> usize {
    message_windows(input).filter_map(Result::ok).count()
}
...
fn collect_order_ids(input: &[u8]) -> Vec<Option<String>> {
    ...
    message_windows_from_reader(reader)
        ...
        .map(|window| {
            // Each window is an MessageWindow; deserialise directly from its segments.
            OrdersMessage::edifact_deserialize(&window.segments)
                .ok()
...
fn demonstrate_error_propagation() {
    ...
    let windows: Vec<_> = message_windows(broken).collect();
    ...
    let results: Vec<_> = message_windows(double_unh).collect();
    assert!(
...
fn main() {
    ...
    for (i, window) in message_windows(interchange)
        .enumerate()
        .map(|(i, r)| (i, r.unwrap()))
    {
